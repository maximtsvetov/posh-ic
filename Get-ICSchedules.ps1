...
#> # }}}3
    ...
    $uri = "$($ICsession.baseURL)/$($ICSession.id)/configuration/schedules"
    ...
    if (![String]::IsNullOrEmpty($properties)) {
        ...
        $uri = "$($ICsession.baseURL)/$($ICSession.id)/configuration/schedules?select=${properties}"
    }
